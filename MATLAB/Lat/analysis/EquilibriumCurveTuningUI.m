...
% all contained in this one file.
    ...
    controller_grid = uigridlayout(controller_tab, [10, 2]);
    controller_grid.ColumnWidth = {'1x', 125};
    controller_grid.RowHeight = { ...
        30, 30, 30, 30, 30, 30, 30, 30, 16, '1x'};
    ...
    fields.ks3 = addNumericField( ...
        controller_grid, 5, 'k_s3  [N/m^3]', defaults.ks3, [-1e9, 1e9], ...
        'Cubic equilibrium-curve error gain.');
    fields.dsdot3 = addNumericField( ...
        controller_grid, 6, 'd_sdot3  [N s^3/m^3]', ...
        defaults.dsdot3, [-1e9, 1e9], ...
        'Cubic equilibrium-curve velocity-error gain.');
    fields.ktheta3 = addNumericField( ...
        controller_grid, 7, 'k_theta3  [N/rad^3]', ...
        defaults.ktheta3, [-1e9, 1e9], ...
        'Cubic lean-angle feedback gain.');
    fields.dthetadot3 = addNumericField( ...
        controller_grid, 8, 'd_thetadot3  [N s^3/rad^3]', ...
        defaults.dthetadot3, [-1e9, 1e9], ...
        'Cubic lean-rate feedback gain.');

    controller_note = uitextarea(controller_grid, ...
        'Editable', 'off', ...
        'Value', { ...
            'Controller:', ...
            'F = m_rod g sin(theta) - k1 s - k_s3 s^3', ...
            '    - d1 s_dot - d_sdot3 s_dot^3', ...
            '    + k_theta theta + k_theta3 theta^3', ...
            '    + d_theta theta_dot + d_thetadot3 theta_dot^3', ...
            's = r - C_eq tan(theta)'}, ...
        'FontName', 'Courier New');
    controller_note.Layout.Row = 10;
    ...
    function renderResult(result, cfg)
        ...
        if result.stopped_early
            ...
            status_label.Text = 'Completed with the current controller gains.';
            status_label.FontColor = [0.10, 0.50, 0.20];
        end
    end

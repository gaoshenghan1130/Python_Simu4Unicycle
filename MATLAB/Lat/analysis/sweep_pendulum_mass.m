% Sweep pendulum/body mass and redesign the LQR controller for every case.
clear; clc; close all;

addpath("param/", "model/");

%% 1. LQR and pendulum-mass sweep settings
...
par_nominal = LatParam();
nominal_m_B = par_nominal.m_B;

% Sweep from 50% to 200% of the current pendulum mass.
% The nominal case (scale = 1) is included and highlighted.
pendulum_mass_scale = 0.50:0.25:2.00;
pendulum_mass_values = nominal_m_B*pendulum_mass_scale;

% By default, this is a pure m_B sweep and I_b is held fixed.
% Set this to true only if the whole pendulum geometry is unchanged and
% its center-of-mass inertia should scale proportionally with mass.
scale_I_b_with_mass = false;
...
number_of_cases = length(pendulum_mass_values);

[~, nominal_case_index] = ...
    min(abs(pendulum_mass_scale-1));

%% 2. Generate colors from light orange to dark red

% Smaller m_B: light orange
% Larger  m_B: dark red
light_color = [1.00, 0.78, 0.55];
dark_color  = [0.55, 0.02, 0.05];
...
closed_loop_poles_all = complex(zeros(number_of_cases, 4));
...
%% 4. Run all pendulum-mass cases

for i = 1:number_of_cases
    par_i = LatParam();

    % Change only the pendulum/body mass.
    par_i.m_B = pendulum_mass_values(i);

    if scale_I_b_with_mass
        par_i.I_b = ...
            par_nominal.I_b*pendulum_mass_scale(i);
    end

    % Re-linearize the system for the current pendulum mass.
    [A_i, B_i] = linearize_lat_model_at_origin(par_i);

    % Redesign the LQR controller for the current pendulum mass.
    ...
    % Store trajectories and parameters.
    t_all{i} = t_i;
    z_all{i} = z_i;
    F_all{i} = F_i;
    par_all{i} = par_i;
...
figure( ...
    'Color', 'w', ...
    'Name', 'Pendulum-mass sweep with redesigned LQR', ...
...
grid(ax_theta, 'on');
box(ax_theta, 'on');
yline(ax_theta, 0, ':', 'HandleVisibility', 'off');
xlabel(ax_theta, 'Time (s)');
ylabel(ax_theta, '$\theta$ (deg)', 'Interpreter', 'latex');
...
grid(ax_r, 'on');
box(ax_r, 'on');
yline(ax_r, 0, ':', 'HandleVisibility', 'off');
xlabel(ax_r, 'Time (s)');
ylabel(ax_r, '$r$ (cm)', 'Interpreter', 'latex');
...
box(ax_theta_dot, 'on');
yline(ax_theta_dot, 0, ':', 'HandleVisibility', 'off');
...
grid(ax_r_dot, 'on');
box(ax_r_dot, 'on');
yline(ax_r_dot, 0, ':', 'HandleVisibility', 'off');
xlabel(ax_r_dot, 'Time (s)');
ylabel(ax_r_dot, '$\dot r$ (m/s)', 'Interpreter', 'latex');
...
grid(ax_force, 'on');
box(ax_force, 'on');
yline(ax_force, 0, ':', 'HandleVisibility', 'off');
xlabel(ax_force, 'Time (s)');
ylabel(ax_force, '$F$ (N)', 'Interpreter', 'latex');
...
grid(ax_phase, 'on');
box(ax_phase, 'on');
xline(ax_phase, 0, ':', 'HandleVisibility', 'off');
yline(ax_phase, 0, ':', 'HandleVisibility', 'off');
xlabel(ax_phase, '$\theta$ (deg)', 'Interpreter', 'latex');
ylabel(ax_phase, '$r$ (cm)', 'Interpreter', 'latex');
title(ax_phase, { ...
    '$\theta$--$r$ relationship', ...
    'solid: simulation, dashed: static equilibrium'}, ...
    'Interpreter', 'latex');

%% Plot every pendulum-mass case
...
for i = 1:number_of_cases
    ...
    % Highlight the current physical model.
    if i == nominal_case_index
        line_width = 3.0;
    else
        line_width = 1.8;
    end
    ...
    case_name = sprintf( ...
        '$m_B=%.3f$ kg ($%.2f\\times$ nominal)', ...
        pendulum_mass_values(i), ...
        pendulum_mass_scale(i));
    ...
    plot(ax_phase, ...
        z_i(:,1)*180/pi, 100*z_i(:,3), ...
        'Color', colors(i,:), ...
        'LineWidth', line_width, ...
        'DisplayName', case_name);

    %% Static equilibrium curve for the current m_B

    % r_eq = [R + G/(m*g)]*tan(theta)
    % where moving_rod_mass = 2*m_L.
    G_i = ...
        par_i.m_B*par_i.g*(par_i.R+par_i.h) ...
        + par_i.m_W*par_i.g*par_i.R;

    moving_rod_mass_i = 2*par_i.m_L;
...
if scale_I_b_with_mass
    inertia_description = 'I_b\ \mathrm{scaled\ with}\ m_B';
else
    inertia_description = 'I_b\ \mathrm{fixed}';
end

title(layout, { ...
    'Pendulum-mass sweep with LQR redesigned for every mass', ...
    sprintf( ...
        ['$Q=\\mathrm{diag}(%g,%g,%g,%g),\\quad ' ...
         'R=%g,\\quad %s$'], ...
        Q(1,1), Q(2,2), Q(3,3), Q(4,4), ...
        R_weight, inertia_description)}, ...
    'Interpreter', 'latex');
...
pendulum_mass_sweep_summary = table( ...
    pendulum_mass_scale(:), ...
    pendulum_mass_values(:), ...
    final_time, ...
...
disp(' ');
disp('Pendulum-mass sweep summary:');
disp(pendulum_mass_sweep_summary);

%% 7. Print closed-loop poles

disp(' ');
disp('Closed-loop poles for every pendulum-mass case:');

for i = 1:number_of_cases
    fprintf( ...
        '\nm_B = %.4f kg (%.2fx nominal)\n', ...
        pendulum_mass_values(i), ...
        pendulum_mass_scale(i));
...
function [A, B] = linearize_lat_model_at_origin(par)
    ...
    % z = [theta, theta_dot, r, r_dot] = 0, F = 0.
